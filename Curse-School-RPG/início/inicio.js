...
function cetro(){
  ...
  if(floresta === "1"){
    alert(`Você se aproxima e enfim percebe ser você, não um espelho, mas uma versão de você, com uma voz metálica que parecia arranhar sua alma, até você a ouve dizer.
      ...
      -"Bom, é raro mas não incomum se ver escolhidos por ai, principalmente de Sobretoure e Modifiers. Mas eu fui expulso, pois além de poder me transformar em tudo temporariamente, se eu devoro um amaldiçoado, minha vida revigora 10 vezes mais."
...
function batalhaClone(){
  ...
  do{
    ...
    if(SuaSorte > SorteDoClone){
      VidaDoClone -= 20
      alert(`O Clone tirou ${SorteDoClone}. Você invoca seu Cetro e lança um feitiço no Clone, sua pele queima como se estivesse no próprio inferno, depois dessa ele demorará para mecher com outro aluno da Curse.
      Agora o clone tem ${VidaDoClone} de vida`)
    }
    else if(SuaSorte < SorteDoClone){
      suaVida -= 20
      alert(`O Clone tirou ${SorteDoClone}. Ele lhe lança um feitiço forte o suficiente para invocar os mortos do campo de batalha, agora um exército de esqueletos saiem do chão sedentos por sangue e só param quando cada um deles lhe acertar um golpe,você fica encurralado por eles e quando todos lhe acertam eles somem, você está cansado e tem ${suaVida} de vida.`)
    }
    else{
  ...
  if(suaVida<= 0){
    alert(`O Clone Ativa sua habilidade especial e invoca uma espada feita de ossos daqueles que já matou, ele corre em volta das árvores que usa como distração chutando galhos em sua direção que você consegue cortar algumas enquanto outras passam, atravessando sua defesa, ele está tão rápido que parece que pode se teleportar, infelizmente você percebe que não tem chance e quando pode ver ele já lhe atravessou com a espada, tudo o que pode ver é uma espada atravessando suas entranhas enquanto você perde a vida, seu último suspiro se esvai junto a uma luz que sai de sua boca representando sua alma. Você morreu!`)
    return
  }
  else{
    alert(`Depois de muito tempo, você ativa sua habilidade especial e Invoca o poder mais poderoso que conhece, junto a ajuda de espiritos que existem em volta de ti, você é capaz de invocar almas de crianças amaldiçoadas que foram vítimas de seu clone, as almas começam a rodear o Clone com raiva e furiosas atravessam seu corpo, assim como ele, foram tirando sua vitalidade aos poucos até não sobrar nada, só o que resta é a carcaça do clone vazia,sem vida e uma espada de ossos das vítimas que ele havia feito, parabéns você o venceu e ele está morto!`)
...
function batalhaSequestro(){
  ...
  do{
    ...
    if(SuaSorte> SorteDoSequestrador){
      ...
      alert(`O Sequestrador tirou ${SorteDoSequestrador}. Você invoca seu Cetro e lança um feitiço no Sequestrador, o cetro brilha ao ponto de esquentar o local como um forno, o Sequestrador podia sentir o calor em sua pele até que percebe um tiro em sua direção, um flash que havia saido do Cetro acerta sua pele a queimando com uma queimadura de 3° grau no minímo. Agora o Sequestrador tem ${VidaDoSequestrador} de vida`)
    }
    else if(SuaSorte < SorteDoSequestrador){
      suaVida -= 20
      alert(`O Sequestrador tirou ${SorteDoSequestrador}. Ele cria diversas adagas feitas de pura lava, claramente era um naturalis renegado, capaz de controlar os elementos, tinha de tomar muito cuidado, assim que percebeu as adagas haviam fincado em sua pele, a queimando e criando uma crosta da pedra esfriando, isso lhe deixa com os movimentos limitados e você tem ${suaVida} de vida.`)
    }
    else{
      alert(`Enquanto você invoca seu cetro, ele cria 100 adagas algumas de gelo e outras de lava, isso te deixa confuso pois não sabe se a temperatura em volta é do local ou das adagas que cancelam seus atributos de esquentar o espaço, isso faz com que desvie com menos facilidade, agora você e ele estão cansados porém nenhum dos dois recebeu nenhum ataque sucedido, Empate!`)
    }
  }
  while(suaVida >0 && VidaDoSequestrador >0)
  if(suaVida<= 0){
    alert(`O Sequestrador Ativa sua habilidade especial e em sua volta vinhas crescem do chão lhe enrolando e absorvendo você até que fique completamente enterrado no chão, deixando apenas sua cabeça para fora para veres o horror que ele faria, com seu poder, ele cria outra adaga de lava e aproxima do pescoço de seu amado, ele dava uma gargalhada enquanto aquele que deveria estar ao seu lado durante a vida chorava de tristeza sabendo que nada poderia ser feito, tudo oque pode fazer foi olhar para ti com ternura e sussurrar entre os lábios para que não se sinta culpado, então o Sequestrador corta a garganta do amor de sua vida e logo depois termina de te enterrar, você morre com a triste lembrança de que ele morreu, aquele que sempre esteve com você foi embora dessa vida na sua frente e você não pôde fazer nada além de observar.`)
      return
  }
  else{
    alert(`Depois de muito tempo, você ativa sua habilidade especial e Invoca o poder mais poderoso que conhece, porém dessa vez é diferente, você sente uma força absurda capaz de explodir até mesmo o sol, você joga seu cetro para cima e ele se torna seu animal, que o usa para lança-lo até o Sequestrador, a espada de ossos que seu inimigo derrotado havia usado surge em suas mãos, contudo os ossos se desfazem e se remontam em uma corrente, você parece saber usa-la mesmo sem treinamento, a arma parece ser feita para suas mão, tanto que quando ela é usada para envolver o inimigo tudo pareceu natural e quando percebeu ele já estava preso em meio as correntes que antes eram uma espada, ele gritava por socorro e você or misericórdia não o mata, mas o envia para o inferno das maldições.`)
...
function matriculaEscolar(){
  ...
  alert(`${nome} do clã ${claPrincipal}, chega a escola após muito tempo de viagem, ele e seu namorado Lucca haviam sido convocados para a escola de maldições na semana passada, Lucca era um escolhido do clã de Modifiers e Sobretoure, enquanto eu apenas do clã de ${claPrincipal}, ao chegarmos o Diretor nos recebeu para nos apresentar a escola, foi um tempo super divertido e criamos ótimos laços com os outros alunos, tudo parecia perfeito, até que as nuvens lá fora começaram a ficar escuras e trovôes ressoaram nos céus, uma forte chuva assolou a academia e uma névoa densa e negra como a escuridou começou a invadir a escola, Lucca e eu ficamos assustados e demos as mãos, mas quando vi, parei de sentir a pele de Lucca, ele havia sumido e a névoa já estava se dissipando.`)
  ...
  if(opcao1 == "1"){
    ...
    if(rolarDados === true){
      ...
      if(sorteioDado >= 0 && sorteioDado <= 10) { 
        ...
        const proposta = prompt(`O diretor o para e lhe pede para ir com ele até seu escritório e você decide segui-lo, ele lhe apresenta alguns cetros e lhe faz uma proposta em especial, você aceitará?? 
        ...
        if(proposta== "1"){
          ...
          alert(`Você insulta e xinga o diretor, ao bater na mochila do mesmo você percebe 5 cetros nela que voam da mochila e batem na parede do escritório, tudo começa a tremer e algumas rachaduras começam a se formar na estrutura da escola, o poder do cetro abalou as paredes, que tremeram tanto ao ponto de derrubar uma estante que havia na sala sob você, o levando a óbito na hora.`)
          return
        }
        else{
          alert(`Você recusa educadamente e a noite volta para roubar a mochila do diretor, ao chegar lá percebe que ela sumiu, oque é bem frustante no final..., enfim decide voltar, mas ao se virar para sair, encontra o reitor o esperando na porta.
          ...
          -"Tudo o que precisava fazer era aceitar minha proposta. Então ele estende a mochila para mim."`)
          cetro()
        }
      }
    }
...
function clas(){
  window.location.href = '../clas/clas.html'
}

function dados(){
  window.location.href = '../dados/dados.html'
}

function escolhidos(){
  window.location.href = '../escolhidos/escolhidos.html'
}

function lideres(){
  window.location.href = '../lideres/lideres.html'
}

function lutas(){
  window.location.href = '../lutas/lutas.html'
}

function morte(){
  window.location.href = '../morte/morte.html'
}

function sanidade(){
  window.location.href = '../sanidade/sanidade.html'
}

function vida(){
  window.location.href = '../vida/vida.html'
}